...
- **データ取得**: 画面規模が小さいため、メモリキャッシュ + useMemo/useCallback で十分と判断し、React Query / SWR は未採用とした。画面やデータ共有が増える場合は、導入してキャッシュ戦略を統一する余地がある。
- **降水量の表示**: 降水量は0未満にならないよう変換時に補正。7日間では曲線の補間で下にはみ出さないよう「降水量」系列は線を直線で描画。
- **チャートの操作性**: ツールチップはグラフより上に固定表示し、グラフを隠しにくくしている。データ点（ドット）はすべて丸で統一し、ホバー・クリック時にはその位置のデータ点がハイライトされ、ツールチップにその時点の値が表示される。
...
3. **既にデプロイ済みのプロジェクト**の場合、環境変数は「追加した時点以降のデプロイ」にしか反映されません。次のどちらかで再デプロイする：
   - **Vercel ダッシュボード**: **Deployments** → 一覧の最新デプロイの **「…」** → **Redeploy** を選ぶ。
   - **Git で push**: 何かコミットを push すると、自動で新しいデプロイが走り、そのときに環境変数が読み込まれる。

両方の環境変数が設定されているときだけ認証が有効になります。未設定（ローカル開発など）では認証せずそのまま表示されます。

**一度認証すると**: 同じブラウザでは、ブラウザがリクエストごとに認証情報を送るため、同じタブ・セッションのあいだは再度パスワードを聞かれません。ブラウザによっては「パスワードを保存」で次回以降も省略できる場合があります。別のブラウザや端末で開くときは、その都度入力が必要です。
...
### グラフ・凡例・ツールチップの改善（完成版）

- **凡例の色対応**: 凡例の色がグラフの線と一致していなかった問題を修正。カスタム凡例を導入し、気温・体感温度・降水量・風速・最高気温・最低気温に固定パレット（SERIES_COLOR_MAP）を割り当て、凡例の丸とグラフの色を統一。
- **凡例の重複解消**: 体感温度などが凡例に 2 件ずつ出ていた問題を修正。Recharts のデフォルト Legend ではなく、データの系列名（`seriesNames`）のみから描画するカスタム凡例に変更し、1 系列につき 1 件だけ表示するようにした。
- **ツールチップの表示位置**: ツールチップが凡例の裏に隠れていたため、z-index を調整（ツールチップ 100、凡例 5）し、常に凡例の手前に表示されるようにした。
- **ツールチップを上に固定**: ツールチップをグラフ上部に固定し、横方向のみカーソルに合わせて動く（ちょこちょこ動く）ように変更。グラフを邪魔しにくく、見やすい位置に統一。
- **ツールチップの見切れ解消**: グラフ領域でツールチップが切れないよう、チャートラッパーの `overflow-hidden` を `overflow-visible` に変更。ツールチップがグラフ外にはみ出しても表示されるようにした。
